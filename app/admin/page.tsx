...
function Secao({
  titulo, descricao, ordem, children,
}: {
  titulo: string;
  descricao: string;
  ordem: number;
  ...
  return (
    <section className="cartao entra" style={{ "--i": ordem, marginBottom: "var(--e3)" } as React.CSSProperties}>
    ...
    <>
      ...
      <main className="pagina">
        ...
        <div className="grade grade-3" style={{ marginBottom: "var(--e5)" }}>
          <div className="cartao cartao-metrica entra" style={{ "--i": 0 } as React.CSSProperties}>
            <div className="metrica-rotulo">Conversões enviadas à Meta</div>
            <div className="metrica-valor num positivo">{d.sentCount}</div>
          </div>
          <div className="cartao cartao-metrica entra" style={{ "--i": 1 } as React.CSSProperties}>
            <div className="metrica-rotulo">Aguardando reenvio</div>
            <div className={`metrica-valor num ${d.pendingEvents.length > 0 ? "negativo" : ""}`}>
              {d.pendingEvents.length}
            </div>
            <div className="metrica-nota">Reenvio automático a cada 5 minutos</div>
          </div>
          <div className="cartao cartao-metrica entra" style={{ "--i": 2 } as React.CSSProperties}>
            <div className="metrica-rotulo">Tempo médio de resposta</div>
            <div className="metrica-valor num">
        ...
        <Secao ordem={5} titulo="Últimos erros" descricao="Falhas no processamento de webhooks, da mais recente pra mais antiga.">
          <table className="tabela">
            <thead>
        ...
        <Secao ordem={6} titulo="Cliques registrados" descricao="Quem passou pelo link de rastreio antes de cair na página de vendas.">
          <table className="tabela">
            <thead>
